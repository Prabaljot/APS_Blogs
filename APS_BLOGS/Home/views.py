...
def userSignup(request):
    if request.method == "POST":
        ...
        if pass1 == pass2:
            ...
            user.save()
            messages.success(request,"Your APS Blogs login account has been successfully created!")
...
def userLogin(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            messages.success(request,"You have successfully logged in into APS Blogs!")
            return redirect('/')
...
def logoutUser(request):
    logout(request)
    messages.success(request,"You've successfully logged out from APS Blogs!")
    return redirect('/')
